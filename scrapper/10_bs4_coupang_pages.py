...
for i in range(1, 6):
    ...
    for item in items:
        ...
        img_url = item.find("img", attrs = {"class" : "search-product-wrap-img"}) # 이미지
        ...
        if total_rating:
            total_rating = total_rating.get_text()[1:-1]
        else:
            # print("<평점 수 없는 제품 제외>")
            continue
        if float(rating) >= 4.5 and int(total_rating) >= 1000:
            ...
            print("가격:", price)
            print("평점:", rating)
            print('평점수:{}'.format(total_rating))
            print("이미지:{}".format(img_url))
            print("------------")
